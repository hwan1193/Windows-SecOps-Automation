...
try {
    ...
    if ($DryRun) { Write-Host "[DRYRUN] Zip $outDir -> $zipPath" }
    else {
...
} catch { Write-Warning "Failed to archive results: $_" }

# 오래된 결과 삭제
if (-not $DryRun -and $RetentionDays -gt 0) {
    $threshold = (Get-Date).AddDays(-$RetentionDays)
    Get-ChildItem -Path $outRoot -Directory |
        Where-Object { $_.Name -match '^\d{4}-\d{2}-\d{2}$' -and $_.LastWriteTime -lt $threshold } |
        ForEach-Object { try { Remove-Item $_.FullName -Recurse -Force; Write-Host "Old report removed: $($_.Name)" } catch {} }
}
